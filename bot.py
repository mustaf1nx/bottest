...
class WelcomeTracker:
    ...
    def __init__(self, max_messages: int = 10, ttl_seconds: float = 900.0) -> None:
        ...
        # Debounce for should_welcome(): new_chat_members (service message)
        # and the chat_member update can both fire for the same join, this
        # stops the person getting greeted twice.
...
async def handle_join_button(
    ...
    try:
        ...
        # Доставка не удалась (сеть, таймаут и т.п.) — не оставляем кнопку
        # висеть в вечной загрузке и не оставляем ссылку "выданной", раз она
        # так и не дошла до человека: следующая попытка должна начать заново.
        LOGGER.exception(
...
async def handle_op_message(
    ...
    if not is_reply and not is_newcomer:
        # Обычное сообщение постороннего/старого участника чата — не имеет
        # отношения к онбордингу, не обрабатываем и не тащим в _thread_messages
        # (иначе список сообщений на удаление рос бы бесконечно для любого,
        # кто просто пишет в чате, и при случайной уборке снёс бы всю его
        # обычную переписку, а не только Q&A с ботом).
        return
